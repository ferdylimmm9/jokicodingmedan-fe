...
interface FormValueState<T extends string[]> {
  keys: [...T];
  children:
    | React.ReactNode
    | ((values: {
...
interface FormValueContextState<T extends string[]> {
  values: {
    //@ts-ignore
    [Key in keyof T as Exclude<T[Key], number>]: any;
  };
...
export default function HOCInput<T extends string[]>(props: FormValueState<T>) {
